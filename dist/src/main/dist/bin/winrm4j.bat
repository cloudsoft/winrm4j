...
REM discover WINRM4J_HOME if not set, by attempting to resolve absolute path of this command (brooklyn.bat)
IF NOT DEFINED WINRM4J_HOME (
    ...
    FOR %%i in ("!WORKING_FOLDER!") DO SET "WINRM4J_HOME=%%~dpi"
)
...
SET INITIAL_CLASSPATH=%WINRM4J_HOME%conf;%WINRM4J_HOME%lib\patch\*;%WINRM4J_HOME%lib\winrm4j\*
...
"%JAVA_BIN%" %JAVA_OPTS% -cp "%INITIAL_CLASSPATH%" io.cloudsoft.winrm4j.winrm.CliClient %*
