...
# discover WINRM4J_HOME if not set, by attempting to resolve absolute path of this command (brooklyn)
if ( $env:WINRM4J_HOME -eq $null ) {
    $WINRM4J_HOME = split-path -parent $ROOT
} else {
    $WINRM4J_HOME = $env:WINRM4J_HOME
}
...
$cp = Get-ChildItem ${WINRM4J_HOME}\conf | Select-Object -ExpandProperty FullName

if ( Test-Path ${WINRM4J_HOME}\lib\patch ) {
    $cp += Get-ChildItem ${WINRM4J_HOME}\lib\patch | Select-Object -ExpandProperty FullName
}

$cp += Get-ChildItem ${WINRM4J_HOME}\lib\winrm4j | Select-Object -ExpandProperty FullName
...
# main class
$javaargs += "io.cloudsoft.winrm4j.winrm.CliClient"
...
# save PID
Set-Content -Path ${WINRM4J_HOME}\pid_java -Value $($process.Id)
